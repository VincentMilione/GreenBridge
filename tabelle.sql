...
id_portafoglio int primary key auto_increment,
credito float,
limite_massimo int default 5000
);

create table agricoltore(
id_agricoltore int primary key auto_increment,
nome varchar(40) not null,
email varchar(50) not null,
pwd varchar(16) not null,
nome_bottega varchar(30) not null,
id_portafoglio int,
foreign key(id_portafoglio) references portafoglio(id_portafoglio)
ON update cascade
on delete cascade
);
...
id_agricoltore int,
foreign key(id_agricoltore) references agricoltore (id_agricoltore)
ON UPDATE CASCADE
ON DELETE CASCADE,
id_admin int NULL,
foreign key(id_admin) references amministratore (id_admin)
ON UPDATE CASCADE
ON DELETE CASCADE
);
